# 业务需求描述
   本系统是一个监测员工行为动态的系统。包含首页统计大屏、人员档案、（新闻社交）态势感知、重点人员库、工作区等五个主要功能。
...
#### 1.1 最上侧展示检测人员总数、重点人员总数、今日新闻数量、新增社交动态的数量。
...
### 2.2 标签筛选 包含最多三级标签，一级标签共六个大类 基本属性、身份属性、组织机构、异常行为、关系属性、行为规律。首次进入系统时，计算每个标签人员数量，并跟随在标签名称后。字体组件样式尽可能小一些，给人员卡片留出位置
